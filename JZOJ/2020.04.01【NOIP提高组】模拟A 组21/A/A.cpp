...
inline unsigned _lowbit(register unsigned x) {
    return x & (-x);
}
inline ULL _max(register unsigned x, register unsigned y) {
    return x > y ? x : y;
}
inline ULL _min(register unsigned x, register unsigned y) {
    return x < y ? x : y;
}
...
signed main() {
    ...
    for (register unsigned i = 1, j, k; i <= N; ++i)
        for (j = i; j <= N; ++j)
            k = j - i + 1,
            res[i][j] =
                k * _max(ST[i][LOG[k]], ST[j - (1 << LOG[k]) + 1][LOG[k]]) +
                sum[i - 1] - sum[j];
